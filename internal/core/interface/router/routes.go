...
func NewRouter(handler *ihttp.Handler) http.Handler {
	...
	router.Handle("POST /submit-post", handler.LogRequest(ihttp.TokenMiddleware(http.HandlerFunc(handler.CreatePost))))
	router.Handle("POST /submit-comment/{id}", handler.LogRequest(ihttp.TokenMiddleware(http.HandlerFunc(handler.CreateComment))))
	return router
}
